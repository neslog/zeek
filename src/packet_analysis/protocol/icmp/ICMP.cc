...
void ICMPAnalyzer::ContinueProcessing(Connection* c, double t, bool is_orig, int remaining, const Packet* pkt)
	...
	if ( packet_contents && len > 0 )
		session_analyzer->PacketContents(data + 8, std::min(len, caplen) - 8);
	...
	// TODO: replace this with packet_analysis forwarding, somehow
	if ( caplen >= len )
		session_analyzer->ForwardPacket(len, data, is_orig, -1, ip.get(), caplen);
